...
void setup(void) 
{
  ...
  bno.setExtCrystalUse(true); // Synchronise the collection of data using the Feather's clock, not the one on-board the BNO.
}
...
void loop() 
{
   ...
   /* 1. Collect <datapts> measurements and store them in strings. */
    while (arr_counter < datapts) {
      ...
      X_pos    = (int) event.orientation.x; // float?
      euler_x += "," + String(X_pos);
      Y_pos    = (int) event.orientation.y; // azimuth, pitch, roll?
      euler_y += "," +  String(Y_pos);
      Z_pos    = (int) event.orientation.z;
      euler_z += "," +  String(Z_pos);
      
      imu::Vector<3> accel = bno.getVector(Adafruit_BNO055::VECTOR_ACCELEROMETER); // event.acceleration.x etc.?
      accel_x += "," + String(accel.x());
      accel_y += "," + String(accel.y());
      accel_z += "," + String(accel.z());
